...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active">{{ __('income.categories') }}</li>
    </ol>
@endsection
...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#categoryCreateModal">
                            {{ __('income.add_category') }} <i class="bi bi-plus"></i>
                        </button>
    ...
    <div class="modal fade" id="categoryCreateModal" tabindex="-1" role="dialog" aria-labelledby="categoryCreateModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="categoryCreateModalLabel">{{ __('income.create_category') }}</h5>
                ...
                <form action="{{ route('income-categories.store') }}" method="POST">
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary">{{ __('income.create') }} <i class="bi bi-check"></i></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
     document.addEventListener('DOMContentLoaded', function () {
        ...
        deleteModal.addEventListener('show.bs.modal', function (event) {
            ...
            var deleteUrl = "{{ route('income-categories.destroy', ':id') }}".replace(':id', userId);
