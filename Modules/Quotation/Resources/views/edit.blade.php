...
@section('title', __('quotation.page_title'))
...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active">{{ __('quotation.form.edit') }}</li>
    </ol>
@endsection
                        ...
                        <form id="quotation-form" action="{{ route('quotations.update', $quotation) }}" method="POST">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="status" id="status" required>
                                            <option {{ $quotation->status == 'Pending' ? 'selected' : '' }} value="Pending">{{ __('quotation.form.status_options.pending') }}</option>
                                            <option {{ $quotation->status == 'Sent' ? 'selected' : '' }} value="Sent">{{ __('quotation.form.status_options.sent') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary">
                                    {{ __('quotation.form.update_quotation') }} <i class="bi bi-check"></i>
                                </button>
                            </div>
                        </form>
                    </div>
